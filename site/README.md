...
Первичная настройка VPS (Ubuntu 24.04) — `setup_server.sh` в корне репозитория: пользователь
`factory`, вход только по SSH-ключу, ufw, fail2ban, swap, Docker и проверка доступов к Telegram,
AITUNNEL и Wildberries. Параметры и порядок запуска — в шапке скрипта. Отчёт о доступах можно
получить повторно: `bash setup_server.sh --check-only`.
